...
private:
    std::vector<std::vector<State<Position>>> matrix;
    int N;
    int M;
...
public:
    ...
    MatrixMaze(unsigned int n, unsigned int m) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            for (int j = 0; j < M; ++j) {
                ...
                StateP stateP(Position(i,j));
                stateP.setCost(randNum);
                inner.push_back(stateP);
            }
    ...
    State<Position>* getInitialState() {
        return &matrix[start.getI()][start.getJ()];
    }
    ...
    std::vector<State<Position>*> getAllPossibleStates(State<Position>* state) {
         ...
         */
        if (j != 0) {
            StateP* left = &matrix[i][j - 1];
            left->setManDist(goal);
            left->setCost(fatherCost + matrix[i][j - 1].getCost());
            if (matrix[i][j - 1].getCost() != -1 && *state->getCameFrom() != *left) {
                left->setCameFrom(state);
                statesVec.emplace_back(left);
            }
        }
        if (j != M - 1) {
            StateP* right = &matrix[i][j + 1];
            right->setManDist(goal);
            right->setCost(fatherCost + matrix[i][j + 1].getCost());
            if (matrix[i][j + 1].getCost() != -1 && *state->getCameFrom() != *right) {
                right->setCameFrom(state);
                statesVec.emplace_back(right);
            }
        }
        if (i != N - 1) {
            StateP* down = &matrix[i + 1][j];
            down->setManDist(goal);
            down->setCost(fatherCost + matrix[i + 1][j].getCost());
            if (matrix[i + 1][j].getCost() != -1 && *state->getCameFrom() != *down) {
                down->setCameFrom(state);
                statesVec.emplace_back(down);
            }
        }
        if (i != 0) {
            StateP* up = &matrix[i - 1][j];
            up->setManDist(goal);
            up->setCost(fatherCost + matrix[i - 1][j].getCost());
            if (matrix[i - 1][j].getCost() != -1 && *state->getCameFrom() != *up) {
    ...
    std::string to_string() {
        ...
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < M; ++j) {
                if (matrix[i][j].getCost() < 10 && matrix[i][j].getCost() > -1) output += ' ';
                output += std::to_string((int)matrix[i][j].getCost());
                if (j < M - 1) output += ',';
            }
            output += '\n';
        }
    ...
    std::string backtrace(State<Position> *goal) {
